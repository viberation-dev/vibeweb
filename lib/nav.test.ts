...
test("exactly one category is active on a category page", () => {
  const directory = SIDEBAR_GROUPS.find((group) => group.label === "Directory · 14")!;
...
test("the Directory label states its own count, and means it", () => {
  // "Directory · 14" is the mockup's label. If a category is ever added to
  // the enum, the label and the list must not drift apart silently.
  // 14 categories plus the "All tools" entry that fronts them.
  const directory = SIDEBAR_GROUPS.find((group) => group.label === "Directory · 14")!;
  assert.equal(directory.items.filter((item) => item.href !== "/tools").length, 14);
});
